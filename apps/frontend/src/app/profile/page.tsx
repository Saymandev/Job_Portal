  ...
  const fetchCVData = useCallback(async () => {
    ...
    if (user) {
      ...
      const userData: CVFormData = {
        ...
        skills: Array.isArray((user as any)?.cvSkills) ? (user as any).cvSkills.map((skill: any, index: number) => ({
          id: skill.id || `skill-${index}`,
          name: skill.name || skill,
          level: skill.level || 'intermediate' as const,
          category: skill.category || 'Technical',
        })) : [],
        experience: Array.isArray((user as any)?.cvExperience) ? (user as any).cvExperience.map((exp: any, index: number) => ({
          id: exp.id || `exp-${index}`,
          title: exp.title || '',
          company: exp.company || '',
        ...
        education: Array.isArray((user as any)?.cvEducation) ? (user as any).cvEducation.map((edu: any, index: number) => ({
          id: edu.id || `edu-${index}`,
          degree: edu.degree || '',
        ...
        certifications: Array.isArray((user as any)?.certifications) ? (user as any).certifications.map((cert: any, index: number) => ({
        ...
        projects: Array.isArray((user as any)?.projects) ? (user as any).projects.map((project: any, index: number) => ({
        ...
        languages: Array.isArray((user as any)?.languages) ? (user as any).languages.map((lang: any, index: number) => ({
